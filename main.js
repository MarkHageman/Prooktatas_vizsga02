...
const hatter = document.getElementsByClassName('modal-hatter')[0];
...
btnYes.addEventListener('click', () => {
    const id = document.getElementById('torolt-termek-id').value;
    sorTorles(id);
})
...
function openModositoModal(id){
    parseInt(document.getElementById(id))
    //Az adatok lekérdezése az azonosító alapján
    const xhr = new XMLHttpRequest();
    xhr.onreadystatechange = () => {
        if(xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200){
            ...
            const modal = document.getElementById('modositoModal');
...
const modositoMModal = document.getElementById('modositoModal');
...
const ujTermekModal = document.getElementById('ujTermekModal');
