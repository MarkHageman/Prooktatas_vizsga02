<?php
    ...
    $termek = new AdatbazisRendezes("localhost", "root", "", "magnahungaria");
    $termekAdat = $termek->rekordLekerdezes('termékek', (int)$id);                   
?>
<!DOCTYPE html>
<html lang="hu">
<head>
...
<body>
    ...
    <h2 class="center p-2"><?php print($termekAdat['cim']); ?></h2>
    <main class="container">
        <div class="row">
            <div id="product">
                <section class="termek-borito p-1 m-3">
                    <img class="p-2 m-1" src=<?php print($termekAdat['borito']); ?> alt="" srcset="">
                </section>
                <section class="termek-adatok p-1 m-3">
                    <h3 class="pt-2 pb-1 m-1"><?php print($termekAdat['cim']) ?></h3>
                    <h4 class="p-1 m-1">Szerző: <?php print($termekAdat['szerzo']) ?></h4>
                    <h6 class="p-1 m-1">ISBN: <?php print($termekAdat['ISBN']) ?></h6>
                    <p class="p-1 m-1">Műfaj: <?php print($termekAdat['mufaj']) ?></p>
                    <p class="p-1 m-1">Kategória: <?php print($termekAdat['mu_tipusa']) ?></p>
                    <p class="p-1 m-1">Megjelenés dátuma: <?php print($termekAdat['kiadas_datum']) ?></p>
                    <p class="p-1 m-1">Ár: <?php print($termekAdat['ar']) ?></p>
                    <?php
                    if((int)$termekAdat['deleted'] !== 0){
                        print('<p class="p-1 m-1">A könyv jelenleg nem rendelhető.</p>');
                    } else {
                        print('<button class="order p-2 mt-2">Rendelés</button>');
                    }
                    ?>
                </section>
                <section id="product-synops" class="p-1 m-3">
                   <p class="col-12 p-1 m-1"><?php print($termekAdat['szinopszis']) ?></p>
                </section>
            </div>
        </div>
    </main>
