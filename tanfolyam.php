<?php session_start(); ?>
<!DOCTYPE html>
<html lang="hu">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Magna Hungaria kiadó</title>
    <link rel="stylesheet" href="./dist/style.css">
    <link rel="shortcut icon" href="./media/favicon.png" type="image/x-icon">
</head>
<body>
    <header>
        <nav class="container-nav p-1">
            <div class="logo">
                <a href="./index.php" class="active" id="logo">
                    <img src="./media/logo.png" alt="">
                    <h5 >Magna </h5>
                    <h5><span id="hun">Hun</span>garia</h5>
                </a>                
                <svg id="hamburger"  class="m-1 p-1">
                    <line class="menuline" x1="2" y1="6" x2="22" y2="6" stroke-width="3" />
                    <line class="menuline" x1="2" y1="12" x2="22" y2="12" stroke-width="3" />
                    <line class="menuline" x1="2" y1="18" x2="22" y2="18" stroke-width="3" />
                </svg>     
            </div>
            <div class="menu">
                <ul>
                    <li><a href="./index.php" class="mx-1 p-2">Főoldal</a></li>
                    <li><a href="./megjelent.php" class="mx-1 p-2">Megjelent kötetek</a></li>
                    <li><a href="./tanfolyam.php" class="mx-1 p-2">Író tanfolyam</a></li>
                    <li><a href="./kapcsolat.php" class="mx-1 p-2">Kapcsolat</a></li>
                    <?php 
                    //ellenőrizzük, hogy a felhasználó bevan-e jelentkezve
                    //ha be van jelentkezve, akkor Kijelentkezés menüpont megjelenítése
                    //ha a felhasználó admin jogosultsággal rendelkezik Raktár menüpont megjelenítése
                    //ha nincs bejelentkezve, Bejelentkezés menüpont megjelenítése
                    if(isset($_SESSION['loggedIn']) && $_SESSION['loggedIn'] === TRUE){
                        print('<li><a href="./kijelentkezes.php" class="mx-1 p-2">Kijelentkezés</a></li>');
                        if((bool)$_SESSION['admin'] === TRUE){
                            print('<li><a href="./raktar.php" class="mx-1 p-2">Raktár</a></li>');
                        }
                    } else {
                        print('<li><a href="./bejelentkezes.php" class="mx-1 p-2">Bejelentkezés</a></li>');
                    }
                    ?>
                </ul>
            </div>
        </nav>
        
    </header>
    <nav class="nav" >
        <ul>
            <li class="m-1 p-3"><a href="./index.php" class="">Főoldal</a></li>
            <li class="m-1 p-3"><a href="./megjelent.php" class="">Megjelent kötetek</a></li>
            <li class="m-1 p-3"><a href="./tanfolyam.php" class="">Író tanfolyam</a></li>
            <li class="m-1 p-3"><a href="./kapcsolat.php" class="">Kapcsolat</a></li>
            <?php 
                    //ellenőrizzük, hogy a felhasználó bevan-e jelentkezve
                    //ha be van jelentkezve, akkor Kijelentkezés menüpont megjelenítése
                    //ha a felhasználó admin jogosultsággal rendelkezik Raktár menüpont megjelenítése
                    //ha nincs bejelentkezve, Bejelentkezés menüpont megjelenítése
                    if(isset($_SESSION['loggedIn']) && $_SESSION['loggedIn'] === TRUE){
                        print('<li class="m-1 p-3"><a href="./kijelentkezes.php">Kijelentkezés</a></li>');
                        if((bool)$_SESSION['admin'] === TRUE){
                            print('<li class="m-1 p-3"><a href="./raktar.php">Raktár</a></li>');
                        }
                    } else {
                        print('<li class="m-1 p-3"><a href="./bejelentkezes.php">Bejelentkezés</a></li>');
                    }
            ?>
        </ul>
    </nav>
    <section class="header-img">
        <h1 class="mx-2">Magna<br>Hungaria</h1>
    </section>

    <main class="container">
        <div class="row main-course">
            <h2 class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 my-3 ">Író tanfolyam</h2>
            <div class="col-12 col-sm-12 col-md-5 col-lg-5 col-xl-5 col-xxl-5 my-3 mx-2 p-1">
                <p>A tanfolyam az alapötlet vázlatba öntésétől kezdve, a karakter és világ formáláson át, a történet mesélés módján keresztül, a párbeszédek megfelelő megfogalmazásától a papírra vetéséig, a különböző szekvenciákat ismerteti meg a hallgatókkal. Ezen belül érintjük a szerkesztési munkához szükséges, valamint a könyv előállításához kapcsolódó szakmák alapfogalmait is. A képzésen a hallgatók házifeladatot (pl. vázlat írás, párbeszéd írás, szöveggondozás, lektori jelentés), valamint dolgozatokat, majd pedig a tanfolyam végén megadott szempontok alapján vizsgafeladatot kapnak.</p>
            </div>
            <div class="col-12 col-sm-12 col-md-5 col-lg-5 col-xl-5 col-xxl-5 my-3 mx-5 px-3 p-1">
                <h3>Tematika</h3>
                <ul>
                    <li class="mb-1">Történetek, versek, dalok eredete, szájhagyomány</li>
                    <li class="mb-1">Vázlat előtti feljegyzések, vázlat alapjai</li>
                    <li class="mb-1">Vázlat ív</li>
                    <li class="mb-1">E/1 vagy E/3 használata, perspektívák, narrációk I.</li>
                    <li class="mb-1">E/1 vagy E/3 használata, perspektívák, narrációk II. (első vizsga)</li>
                    <li class="mb-1">Mű elemzések (pl. Jókai, Petőfi, Ady)</li>
                    <li class="mb-1">Hogy dolgoznak a lektorok?</li>
                    <li class="mb-1">Nyomdai ismeretek, könyvterjesztés</li>
                    <li class="mb-1">Szerzői jogi ismeretek, alapfogalmak (második vizsga)</li>
                    <li class="mb-1">Első vizsga elemzése</li>
                    <li class="mb-1">Realisztikus karakterek alkotása</li>
                    <li class="mb-1">Történet ív, avagy hogy öntsük szövegbe a vázlatot</li>
                    <li class="mb-0">Záróvizsga(egy kész mű bemutatása (regény esetén első 60 oldal), írásbeli vizsga)</li>
                </ul>
            </div>
        
        </div>
    </main>

    <article class="container">
        <div class="row course-datas">
            <h2 class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 m-3">A tavaszi képzés</h2>
            <div class="time col-12 col-sm-12 col-md-5 col-lg-5 col-xl-5 col-xxl-5 mx-2 my-3">
                <p class="mb-2"><strong>Tájékoztató, beiratkozás:</strong> 2023. február 08. (szerda) 16 óra</p>
                <p class="mb-2"><strong>Kezdés:</strong> 2023. február 15. (szerda)</p>
            </div>
            <div class="length col-12 col-sm-12 col-md-5 col-lg-5 col-xl-5 col-xxl-5 mx-2 my-3">
                <p class="mb-2"><strong>Időtartam:</strong> 135 óra, heti 2 * 3 óra (45 perces órák)</p>
                <p class="mb-2"><strong>Tandíj:</strong> 200 000 Ft (5 * 40000 Ft részletfizetéssel, havonta</p>
            </div>
            
        </div>
    </article>

    
    <form class="container regist"  action="./tanfolyam.php" method="post">
        <h2 class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 p-3">Jelentkezés a tanfolyamra</h2>            
            <div class="my-1">
                <label for="name">Név<sup>*</sup></label><br>
                <input class="" type="text" name="name" id="name" required>
            </div>
            <div class="my-1">
                <label for="email">E-mail cím<sup>*</sup></label><br>
                <input class="" type="email" name="email" id="email" required>
            </div>
            <div class="my-1">
                <label for="tel">Telefonszám<sup>*</sup></label><br>
                <input class="" type="tel" name="tel" id="tel" required>
            </div>
            <div class="my-1">
                <label for="message">Üzenet</label><br>
                <textarea class="" name="message" id="message" rows="5"></textarea>
            </div>
            <div>
                <input class="p-2" type="submit" value="JELENTKEZÉS">
            </div>
        
        


    </form>
      

    


    <footer class="container-footer ">
        <div class="container">
            <div class="row">
                <div class="datas m-3 col-12 col-sm-5 col-md-5 col-lg-3 col-xl-3 col-xxl-3">
                    <h4><strong>Magna Hungaria Bt.</strong></h4>
                    <p class="m-1 p-1"><strong>Cím:</strong><br> <a href="">2800 Tatabánya, Tas vezér u. 9<br>3/12.(kaputelefon: 12) </a></p>
                    <p class="m-1 p-1"><strong>Tel:</strong><br> <a href="tel:+06903219801">06-90-321-9801</a></p>
                    <p class="m-1 p-1"><strong>E-mail:</strong><br> <a href="mailto:info@magnahungaria.hu">info@magnahungaria.hu</a></p>
                    <p class="m-1 p-1"><strong>Facebook:</strong><br> <a href="https://www.facebook.com/MagnaHungaria">/Magna Hungaria</a></p>
                </div>
                <div class="sitemap m-3 col-12 col-sm-5 col-md-5 col-lg-3 col-xl-3 col-xxl-3">
                    <h4><strong>Honlaptérkép</strong></h4>
                    <p class="m-1 p-1"><a href="./index.php"  >Főoldal</a></p>
                    <p class="m-1 p-1"><a href="./megjelent.php">Megjelent kötetek</a></p>
                    <p class="m-1 p-1"><a href="./kapcsolat.php">Kapcsolat</a></p>
                </div>
                <div class="course m-3 col-12 col-sm-5 col-md-5 col-lg-3 col-xl-3 col-xxl-3">
                    <h4><strong>Tanfolyamunk</strong></h4>
                    <p class="m-1 p-1"><a href="./tanfolyam.php">Író tanfolyam</a></p>
                </div>
                
            </div>
            <p class="text-bottom">© 2022 Magna Hungaria Bt. – Minden jog fenntartva. | <a href="">Adatkezelési tájékoztató</a></p>
        </div>
        
        
    </footer>




<script src="./main.js"></script>
</body>
</html>